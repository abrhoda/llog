...
// the log_file
void add_log_file(const char* name, struct llog_log_file* log_file,
  ...
  log_file->rotation_policy =
      NULL;  // don't allow existing policies without any validation.
...
void close_log_files(void) {
  ...
  while (file_count < LLOG_FILES_LENGTH && llog.files[file_count] != NULL) {
    // TODO check if `llog.files[file_count]->file == NULL` first. shouldnt
...
static int rotate_file(struct llog_log_file* log_file) {
  ...
  snprintf(target, sizeof(target), "%s-%d", log_file->name,
...
static void write_to_stderr(struct llog_log_event* event) {
  ...
  if (offset >= LLOG_LINE_LENGTH) {
    // TODO properly handle this error, although it should only be possible if
    // log_line_buffer size gets set very small or filename is near linux
    // filename max length and near line number 1M+ of a file.
    return;
  }
  offset +=
  ...
  if (offset >= LLOG_LINE_LENGTH) {
    // TODO properly handle this error. This means that the log message was
    // trunctated!
    return;
  }
...
static void write_to_files(struct llog_log_event* event) {
  ...
  if (offset == 0) {
    // TODO properly handle this error, although this hsould be impossible if
    // log_line_buffer size gets set very small.
    return;
  }
  offset += snprintf(log_line_buffer + offset, sizeof(log_line_buffer) - offset,
                     "%-5s %s:%d ", log_level_strings[event->level],
  ...
  for (iter = 0; iter < llog.files_count; ++iter) {
    ...
    // check if the file needs to rotate and rotate if needed. +1 byte size
    // accounts for \n
...
void llog_log(enum log_level log_level, const char* file, int line,
  ...
  // need to reparse varargs each `write_to_...` fn because vprintf and family
  // can invalidate the va_list
